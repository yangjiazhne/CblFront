...
let Overlay = function (viewer, staticCanvas, fabricCanvasOptions = {}) {
  ...
  if (!this._canvas || !this._canvasdiv) {
    console.error('Canvas or CanvasDiv is not initialized properly');
    return;
  }
  ...
  if (!this._fabricCanvas) {
    console.error('Fabric.js Canvas failed to initialize');
    return;
  }
  ...
  this._viewer.addHandler('destroy', () => {
    window.removeEventListener('resize', this.resize);
  });

  this._viewer.addHandler('open', () => {
    const overlay = this._viewer.fabricjsOverlay();
    overlay.resize();
...
Overlay.prototype = {
  ...
  resize: function () {
    if (!this._viewer || !this._viewer.container) {
      console.warn('Viewer or container is not initialized or has been destroyed.');
      return;
    }
  ...
  resizeCanvas: function () {
    if (!this._fabricCanvas || !this._canvasdiv) {
      console.warn('Fabric.js canvas or container is not initialized.');
      return;
    }
